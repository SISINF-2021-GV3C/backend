...
class User {
    ...
    static async create(firstName,lastName,email,username,password,country,genero,tlf,date){
        var checkNickname = await this.getUserByNickname(username)
       
        if(!checkNickname){
            var sql = "INSERT INTO usuario(nombre,apellido,email,nickname,pass,pais,genero,telefono,anyo_nac) values  (\"" +
            firstName + "\", \"" +
            lastName + "\", \"" +
            email + "\",  \"" +
            username + "\", \"" +
